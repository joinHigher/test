    ...
    <div class="coupon-temp">
        ...
        <div class="el-container">
            <div class="el-sub-container text-right">
                <router-link :to="{name: 'marketing_coupon_couponAdd'}" class="el-button el-button--primary">
                    新增模板
                </router-link>
            </div>
            <el-table border :data="tableData" v-loading="tableLoading">
                ...
                <el-table-column label="操作">
                    <template scope="scope">
                        <el-button v-if="self.showAction_('promotion/template/delete-coupon')" type="danger" size="small" @click="removeItem(scope.row)">删除</el-button>
                        <el-button v-if="self.showAction_('promotion/template/setting-couponsite')" type="primary" size="small" @click="changeSiteOpt(scope.row)">配置站点</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <!-- 弹窗S -->
        <el-dialog title="配置站点" v-model="siteDialog" size="tiny" class="">
...
export default {
    ...
    methods: {
        ...
        changeSiteOpt (row) {
            this.batchSet = '';
            for (let j in this.siteOpt) {
                this.siteOpt[j].status = '0';
            }
        ...
        removeItem (row) {
            this.$confirm('是否删除该模板，删除模板后只之前生成的Coupon不受影响?', '删除模板', {
                type: 'warning'
            }).then(() => {
