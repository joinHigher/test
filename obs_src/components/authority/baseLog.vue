    ...
    <div>
        ...
        <div class="el-container">
            <div class="el-sub-container text-right">
                <el-button type="primary" v-if="self.showAction_('base/admin-log/request-route2name')" @click="setOpt">请求路由对应操作配置</el-button>
            </div>
            <el-table :border="true" :data="tableData" v-loading="tableLoading">
                <el-table-column prop="id" label="日志ID"></el-table-column>
                <el-table-column label="操作时间">
                    <template scope="scope">{{self.showDate(scope.row.create_time)}}</template>
                </el-table-column>
                ...
                <el-table-column label="日志详情">
                    <template scope="scope">
                        <el-button type="primary" size="small" v-if="self.showAction_('base/admin-log/detail')" @click="getInfo(scope.row.id)">详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <el-dialog title="配置" v-model="optDialog" size="tiny">
            <el-form :model="optForm" ref="optForm" label-width="100px">
                <el-form-item label="配置">
                    <el-input type="textarea" :rows="12" v-model="optForm.request_route2name"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="saveOpt" v-loading="saveLoading" :disabled="saveLoading">确 定</el-button>
                <el-button type="danger" @click="optDialog = false">取 消</el-button>
            </span>
        </el-dialog>
        <!-- 弹窗E -->
    </div>
...
export default {
    ...
    methods: {
        showDate (date) {
            return this.timeZone_(date, this.$store.getters.getTimeZone);
        },
        getData (search) {
            if (search === 'search') {
                this.option.page = 1;
            ...
            this.$http.get('base/admin-log/list', {params: this.option}).then(res => {
        ...
        getInfo (id) {
            this.$http.get('base/admin-log/detail', {params: {id: id}}).then(res => {
                if (res.body.code == 0) {
        ...
        setOpt () {
            this.$http.get('base/admin-log/request-route2name').then(res => {
                if (res.body.code === 0) {
                    this.optForm.request_route2name = res.body.data;
                    this.optDialog = true;
                } else {
        ...
        saveOpt () {
            ...
            var params = this.optForm.request_route2name;
            this.$http.post('base/admin-log/save-request-route2name', {request_route2name: params}).then(res => {
