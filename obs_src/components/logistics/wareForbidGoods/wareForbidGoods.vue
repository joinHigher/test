  ...
  <div>
    ...
    <div class="el-container">
      <div class="el-sub-container text-right">
        ...
        <template>
          <router-link :to="{name: 'newWareForbidGoods', query: {type: 1}}">
            <el-button type="primary" icon="plus" v-if="self.showAction_('logistics/warehouse-forbid-attrs/add')">新建规则</el-button>
          </router-link>
        </template>
      </div>
      <el-table ref="multipleTable" border :data="tableData" v-loading="tableLoading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center">
          <template scope="scope">
            <router-link :to="{name: 'editWareForbidGoods',query: {type: 2, row: JSON.stringify(scope.row)}}">
              <el-button type="primary" v-if="self.showAction_('logistics/warehouse-forbid-attrs/edit')">编辑</el-button>
            </router-link>
            <el-button type="danger"  @click="removeItem(scope.row)" v-if="self.showAction_('logistics/warehouse-forbid-attrs/delete')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted () {
        ...
        this.countryList = this.getCounryListFun();
        this.pipelineList = this.getPipe();
    },
    methods: {
      getData () {
        this.tableLoading = true;
        this.$http.get('logistics/warehouse-forbid-attrs/list', {params: this.option}).then(res => {
      ...
      removeItem (row) {
        this.$confirm('是否删除该项?', '提示', {
          ...
          this.$http.post('logistics/warehouse-forbid-attrs/delete', {id: row.id}).then(res => {
            if (res.body.code === 0) {
