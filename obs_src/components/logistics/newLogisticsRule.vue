  ...
  <div>
    <div class="el-container">
      <el-form :model="option" label-width="200px" ref="costForm">
        ...
        <el-form-item label="已设定条件">
          <el-row   v-for="(item , index) in  option.ruleItem" :key="index" class="ruleItem" >
            <el-col :span="2">
        ...
        <el-form-item label="优先（必须）使用物流">
          <el-row>
            <el-col :span="5">
              <el-input v-model="option.logisticsValidCode" @focus="clickValidCode" readonly></el-input>
            </el-col>
          </el-row>
        </el-form-item>
        <el-form-item label="不可使用物流">
          <el-row>
            <el-col :span="5">
              <el-input v-model="option.logisticsInvalidCode" @focus="clickInValidCode" readonly></el-input>
            </el-col>
          </el-row>
        </el-form-item>
    ...
    <el-dialog title="产品性质" v-model="proNatureDialog" class="log-group" size="small">
      <el-form :model="proNatureForm" label-width="50px">
        ...
        <el-form-item>
          <el-tree :data="proNatureForm.proNatureList" :show-checkbox="true" default-expand-all node-key="propertyCode"
                   ref="proNatureFormTree" highlight-current :props="proNatureForm.defaultProps"  class="proTypeTree" v-loading = "proNatureListLoading">
          </el-tree>
        </el-form-item>
        <el-form-item label="类型">
    ...
    methods: {
      ...
      newEditCountry (){
        ...
        this.$http.get('logistics/country-info/list',{params:{page_size:2000,page:1,countryCode:''}}).then(res =>{
          ...
          //获得当前记录的生效国家
          this.initEffectCountry(this.option.countrysName);
      ...
      newEditSite (){
        ...
        this.effectSiteForm.siteCheckAll = false;
      ...
      handleCheckedSite (value){
        ...
        this.effectSiteForm.siteCheckAll = checkedCount === this.effectSiteForm.siteNamesArr.length;
        this.effectSiteForm.isIndeterminate = checkedCount > 0 && checkedCount < this.effectSiteForm.siteNamesArr.length;
        var arr = [];
        for(var i=0;i<this.effectSiteForm.checkedSites.length;i++){
            if(this.effectSiteForm.checkedSites[i] != ''){
                arr.push(this.effectSiteForm.checkedSites[i]);
            }
        }
        this.effectSiteForm.checkedSites = arr;
      },
      //生效网站 保存
      effectSiteDialogSave (){
        ...
        //console.log('生效网站: ' + this.option.sitesName);
        for(var i=0;i<namesArr.length;i++){
      ...
      editRuleSave (){
        ...
        for(var i=0;i<this.selectValidCodesData.length;i++){
          for(var j=0;j<this.selectInValidCodesData.length;j++){
            if(this.selectValidCodesData[i].logisticsCode === this.selectInValidCodesData[j].logisticsCode){
        ...
        this.$http.post('logistics/rule-info/add', this.option).then(res => {
          if (res.body.code === 0) {
            this.$message({
              type: 'success',
              message: '保存成功'
