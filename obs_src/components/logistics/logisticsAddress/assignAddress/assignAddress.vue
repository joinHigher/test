  ...
  <div>
    ...
    <div class="el-container">
      <div class="el-sub-container text-right">
        <el-button type="primary" @click="$router.push({name: 'newAssignAddress'})" icon="plus">添加</el-button>
      </div>
      <el-table ref="multipleTable" border :data="tableData" v-loading="tableLoading" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="tips" label="提示语"></el-table-column>
        ...
        <el-table-column label="操作">
          <template scope="scope">
            <router-link :to="{name: 'editAssignAddress',query: {row: JSON.stringify(scope.row)}}">
  ...
  export default {
    ...
    methods: {
      ...
      getData () {
        ...
        this.$http.get('logistics/appoint-address/list',{params:this.option}).then(res => {
      ...
      removeItem (row) {
        this.$confirm('是否删除该项?', '提示', {
          ...
          this.$http.post('logistics/appoint-address/delete', {id: row.id}).then(res => {
            if (res.body.code === 0) {
