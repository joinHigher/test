    ...
    export default {
        ...
        methods: {
            initPage () {
                ...
                this.option.name = this.row.name;
                this.option.logisticsCode = this.row.logisticsCode;
                this.option.countryNames = this.row.countryNames;
                this.dialogObj.countryCodesSelected = this.row.countryCodes.slice(0);
                this.row.itemList.forEach(item => {
                    this.conditionForm.checkList.push(item.category.toString());
                });
                this.condResArr.forEach(cond => {
                    this.row.itemList.forEach( rowItemList => {
                        if (cond.category === Number(rowItemList.category)) {
                            let attrValTempArr = rowItemList.attrVal.split('-');
                            cond.beginCount = attrValTempArr[0];
                            cond.endCount = attrValTempArr[1];
                            cond.show = true;
                            let tipsTemp = JSON.parse(rowItemList.tips);
                            for (let i in tipsTemp) {
                                 let objTemp = {lang: i, noticeTxt: tipsTemp[i]};
                                 cond.tips.push(objTemp);
                            }
                        }
                    });
                });
            ...
            ruleFormSave () {
                ...
                console.log(params);
                this.$http.post('logistics/address-rule/edit', params).then(res => {
                    if (res.body.code === 0) {
